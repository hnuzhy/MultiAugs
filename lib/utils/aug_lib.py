...
def set_augmentation_space(augmentation_space, num_strengths, custom_augmentation_space_augs=None):
    ...
    if 'xlong' in augmentation_space:
        ...
        ### Apply the TrivialAugment for SSHPE
        ### We need to remove keypoint-sensitive transforms
        ALL_TRANSFORMS = [
            identity,
            auto_contrast,
            equalize,
...
class TrivialAugment:
    def __call__(self, img):
        img = Image.fromarray(img)  # transform CV2 image in our project into PIL image
