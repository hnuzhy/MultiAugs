...
def train(config, train_loader, model, criterion, optimizer, epoch,
    ...
    # define and update the aug_couple for [teacher_net, student_net]
    begin_e, end_e = config.TRAIN.BEGIN_EPOCH, config.TRAIN.END_EPOCH 
    if config.PROG_MODE != "":  # progressively updating with epoch index
        aug_couple_list = config.AUG_EASY_HARD_COUPLES[config.PROG_MODE]
        index_e_float = ((epoch - begin_e) / (end_e - begin_e)) * len(aug_couple_list)
        aug_couple = aug_couple_list[int(np.floor(index_e_float))]
    else:  # the default old training way
        if config.USE_MixUp_AUG:            aug_couple = ["A30", "MU"]
        elif config.USE_Cutout_AUG:         aug_couple = ["A30", "CO"]
        elif config.USE_CutMix_AUG:         aug_couple = ["A30", "CM"]
        elif config.USE_JointCutout_AUG:    aug_couple = ["A30", "JC"]
        elif config.USE_JointCutMix_AUG:    aug_couple = ["A30", "JO"]
        else:                               aug_couple = ["A30", "A60"]
    
    if not config.STEP_WISE and config.PROG_MODE != "":
        msg = '(Epoch-Wise) Epochs:[%d]/[%d], \t aug_couple: [%s, %s]'%(
            epoch,  end_e - begin_e, aug_couple[0], aug_couple[1])
        logger.info(msg)
    ...
    for i, (input, target, target_weight, meta) in enumerate(train_loader):
        ...
        if config.STEP_WISE and config.PROG_MODE != "":  # (Steps-Wise)
            index_e_float = (i / len(train_loader)) * len(aug_couple_list)
            aug_couple = aug_couple_list[int(np.floor(index_e_float))]
        
        # compute output
        # output = model(input, aug_couple)  # 2023-07-17: this has bug for progressive training
        output = model(input, aug_couple=aug_couple)
