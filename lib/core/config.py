...
## RandAugment. Using this way is too slow ...
config.CONS_RAND_AUG = False
config.RAND_MAGNITUDE = 20
## Joint Cutout
config.MASK_JOINT_NUM = 5
## Semi-supervised Cut-Occlude (SSCO)
config.KP_CutMix_JOINT_NUM = 2
# config.KP_CutMix_JOINT_NUM = 5
## Cutout and CutMix
config.MASK_HOLES_NUM_CO = 5
...
config.USE_CutMix_AUG = False
config.USE_RandAug_AUG = False
config.USE_JointCutout_AUG = False
config.USE_JointCutMix_AUG = False

config.USE_JointCutMix_CutOut_AUG = False
config.USE_JointCutout_CutMix_AUG = False

config.EXP_SUB_NAME = ''

# progressively update the Easy-Augmentation of teacher-net and Hard-Augmentation of student-net (do not work!!!)
config.STEP_WISE = False  # progressively updating by stepwise or epochwise (default is epochwise) (do not work!!!)
config.PROG_MODE = "PROG_1"
config.AUG_EASY_HARD_COUPLES = {
    "PROG_1": [["A30", "A60"], ["A30", "MU"], ["A30", "CM"], ["A30", "CO"], ["A30", "JO"], ["A30", "JC"]],
    "PROG_2": [["A30", "JC"], ["A30", "JO"], ["A30", "CO"], ["A30", "CM"], ["A30", "MU"], ["A30", "A60"]],
    "PROG_3": [["A30", "A60"], ["A60", "MU"], ["MU", "CM"], ["CM", "CO"], ["CO", "JO"], ["JO", "JC"]],
    "PROG_4": [["JO", "JC"], ["CO", "JO"], ["CM", "CO"], ["MU", "CM"], ["A60", "MU"], ["A30", "A60"]],

    "PROG_5": [["A30", "A60"], ["A30", "CM"], ["A30", "JC"], ["A30", "JC+CM"]],
    "PROG_6": [["A30", "A60"], ["A30", "CO"], ["A30", "JO"], ["A30", "JO+CO"]],
    "PROG_7": [["A30", "JC+CM"], ["A30", "JC"], ["A30", "CM"], ["A30", "A60"]],
    "PROG_8": [["A30", "JO+CO"], ["A30", "JO"], ["A30", "CO"], ["A30", "A60"]],
    }

config.MULTI_AUGS = False  # multiple unsupervised losses by applying multiple separate augs
config.SHARPEN_PRED = False  # regularizing (sharpening) the predictions to have low entropy
config.HEAT_FUSION = False  # sum + avg fusion of multiple consistency prediction heatmaps
config.CONF_MASKING = False  # mask out examples that the current model is not confident about
config.CONF_THRESH = 0.5  # confident threshold for conf_masking
config.REPEAT_AUG = False  # multiple unsupervised losses by applying single aug repeatly
config.REPEAT_TIMES = 2  # repeat times for single aug in repeat_aug
